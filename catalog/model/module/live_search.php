...
namespace Opencart\Catalog\Model\Extension\LiveSearch\Module;

class LiveSearch extends \Opencart\System\Engine\Model {
    ...
    public function loadDependencies() {
        ...
        $extension = json_decode(file_get_contents(DIR_EXTENSION . "live_search/install.json"), true);
    ...
    public function search($text, $searches = array(), $research=0) {
        ...
        foreach ($searches as $search) {
            if (is_file(DIR_OPENCART . 'extension/live_search/system/config/live_search/'.$search.'.php')) {
                $this->config->addPath(DIR_OPENCART . 'extension/live_search/system/config/');
                $this->load->config('live_search/' . $search);
                $search_filter[$search] =  ($this->config->get('live_search_' . $search));
            } else {
...
//what table to search and join tables
        ...
        foreach ($searches as $kek => $search) {
            ...
            foreach ($query->rows as $key => $row) {
                ...
                if ((isset($product_ides[$search]) && in_array($row[$search . '_id'], $product_ides[$search]['id'])) || (isset($product_ides[$search]) && !empty($row['image'])  && $settings['no_dublicate_images'] && in_array($row['image'], $product_ides[$search]['image']))  ) {
                    ...
                    $product_ides[$search]['image'][]             = isset($row['image']) && !empty($row['image']) ? $this->model_tool_image->resize($row['image'], $settings['image_width'], $settings['image_height']) : $this->model_tool_image->resize('catalog/live_search/no_image_search.png', $settings['image_width'], $settings['image_height']);;
                    ...
                    $result[$search][$key]['image']         = isset($row['image']) && !empty($row['image']) ? $this->model_tool_image->resize($row['image'], $settings['image_width'], $settings['image_height']) : $this->model_tool_image->resize('catalog/live_search/no_image_search.png', $settings['image_width'], $settings['image_height']);
                    ...
                    if ($search == 'category') {
                        $result[$search][$key]['href'] = $this->url->link('product/' . $search .'&language=' . $this->config->get('config_language'), 'path=' . $row[$search . '_id']);
                    } else if ($search == 'manufacturer') {
                        $result[$search][$key]['href'] = $this->url->link('product/' . $search . '/info&language=' . $this->config->get('config_language'), $search . '_id=' . $row[$search . '_id']);
                    } else if ($search == 'post') {
                        $result[$search][$key]['href'] = $this->url->link('extension/d_blog_module/post', $search . '_id=' . $row[$search . '_id']);
                    } else {
                        $result[$search][$key]['href'] = $this->url->link($search . '/' . $search . '&language=' . $this->config->get('config_language'), $search . '_id=' . $row[$search . '_id']);
                    }
                    if ($settings['price'] == 0) {
    ...
    public function autocomplite($keyword){
        $setting1 = $this->model_setting_setting->getSetting('live_search');
        $settings = $setting1['live_search_setting'];
